...
describe("pdas", () => {
    ...
    async function derivePda(color: string, pubkey: anchor.web3.PublicKey) {
        let [pda, _] = await anchor.web3.findProgramAddress(
            [
                pubkey.toBufer(),
                Buffer.from("_"),
                Buffer.from(color),
    ...
    async function createLedgerAccount(color: string, 
    ...
    async function modifyLedger(
        ...
        try {
            ...
            console.log("The account exsist.");
        } catch(e) {
            console.log("The account does not exsist");
